...
class CustomLoan(AccountsController):
	def on_update(self):
...
def update_total_amount_paid(doc):
	...
	frappe.db.set_value("Custom Loan", doc.name, "total_amount_paid", total_amount_paid)
...
def send_xml_to_server(data):
	...
	CustomLoan = frappe.get_doc("Custom Loan", data)
	...
	for schedule in CustomLoan.repayment_schedule:
...
def close_unsecured_term_loan(loan):
	loan_details = frappe.db.get_value(
		"Custom Loan", {"name": loan}, ["status", "is_term_loan", "is_secured_loan"], as_dict=1
	)
	...
	if (
		...
		frappe.db.set_value("Custom Loan", loan, "status", "Closed")
	else:
		frappe.throw(_("Cannot close this loan until full repayment"))


def close_loan(loan, total_amount_paid):
	frappe.db.set_value("Custom Loan", loan, "total_amount_paid", total_amount_paid)
	frappe.db.set_value("Custom Loan", loan, "status", "Closed")
...
def unpledge_security(
	...
	if loan:
		...
		loan_doc = frappe.get_doc("Custom Loan", loan)
...
def get_shortfall_applicants():
	loans = frappe.get_all("Loan Security Shortfall", {"status": "Pending"}, pluck="Custom Loan")
	applicants = set(frappe.get_all("Custom Loan", {"name": ("in", loans)}, pluck="name"))
...
def make_refund_jv(loan, amount=0, reference_number=None, reference_date=None, submit=0):
	...
	loan_details.doctype = "Custom Loan"
